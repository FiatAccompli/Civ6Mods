...
-- Overflow: is what it is - the game's handling of overflow is outside of mod control
...
-- deep in the dll (Why not in lua?  How to put this nicely?  Because Firaxis programmers are shit at their job.)
-- there is nothing to do besides using this somewhat less clean method of implementation.
...
-- it's probably ok post R&F as trade routes are gated by the level 1 building and not on building the district itself.
...
-- it was with COST_PROGRESSION_NUM_UNDER_AVG_PLUS_TECH.
...
-- Apply the same 3x cost to aerodrome replacements (although there are not currently any unless another mod adds a custom one).
UPDATE Districts SET 
    Cost = Cost * 2,
	CostProgressionParam1 = CostProgressionParam1 * 2
WHERE EXISTS 
...
--  MBANZA
-- 
-- Cost multiplier is 2x since aqueduct/baths come slightly later in the game than most specialty districts and this keeps the 
