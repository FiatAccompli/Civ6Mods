...
local m_additionalPanels :table = {}

function RealizeEmptyMessage(panelsVisible:boolean)
  local customVisible = false;
  for _, v in pairs(m_additionalPanels) do 
    customVisible = customVisible or v;
  end
	if(m_hideChat and m_hideCivics and m_hideResearch and not customVisible and not panelsVisible) then
...
function CreateAdditionalPanel(addin:table)
  ...
  checkBox.Check:GetTextButton():SetText(Locale.Lookup("LOC_UI_PLUGINS_PANEL_CHECK_FORMATTER", id));
  checkBox.Check:SetCheck(true);
  m_additionalPanels[panel] = true;

  checkBox.Check:RegisterCheckHandler(
      function() 
        local visible = checkBox.Check:IsChecked();
        panel.MainPanel:SetHide(not visible);
        m_additionalPanels[panel] = visible;
        RealizeEmptyMessage();
      end);
end
...
function Initialize()
  ...
  for _, addin in ipairs(Modding.GetUserInterfaces("InGame_WorldTracker")) do
    CreateAdditionalPanel(addin);
	end
