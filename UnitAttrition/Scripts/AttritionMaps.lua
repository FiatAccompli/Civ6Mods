...
-- whether it has a river on its east, southeast or southwest edge so define some helpers functions so we can treat 
...
-- Map from diplomatic state name to index, of only the values we care about.
local DIPLO_STATE_NAME_TO_INDEX_MAP = {};
for state in GameInfo.DiplomaticStates() do
  DIPLO_STATE_NAME_TO_INDEX_MAP[state.StateType] = state.Index; 
end

-- Cached constants for the diplo states we care about
local ALLIED_DIPLO_STATE_INDEX = DIPLO_STATE_NAME_TO_INDEX_MAP['DIPLO_STATE_ALLIED'];
local FRIEND_DIPLO_STATE_INDEX = DIPLO_STATE_NAME_TO_INDEX_MAP['DIPLO_STATE_DECLARED_FRIEND'];
local WAR_DIPLO_STATE_INDEX = DIPLO_STATE_NAME_TO_INDEX_MAP['DIPLO_STATE_WAR'];
local WAR_WITH_MINOR_DIPLO_STATE_INDEX = DIPLO_STATE_NAME_TO_INDEX_MAP['DIPLO_STATE_WAR_WITH_MINOR'];
local SUZERAIN_DIPLO_STATE_INDEX = DIPLO_STATE_NAME_TO_INDEX_MAP['DIPLO_STATE_MAX_INFLUENCE'];
...
for _, type in ipairs(HARBOR_DISTRICT_TYPE_INDEXES) do 
  print("harbor", type)
end
...
-- Returns a table from playerId to whether rooute use is allowed in plots owned by that player.
-- Arguments:
--  player: A Player object for whom the route use is to be calculated.
...
local function GetRouteUseMap(player, attritionSpec)
  ...
  for _, minor in ipairs(PlayerManager.GetAliveMinors()) do
    ...
    if attritionSpec.RoutesInCityStateSuzerain and diplomaticState == SUZERAIN_DIPLO_STATE_INDEX then
  ...
  for _, major in ipairs(PlayerManager.GetAliveMajors()) do
    ...
    print("Diplo state from: ", player:GetID(), majorId, diplomaticState);
  ...
  canUseRoutesByOwner[player:GetID()] = attritionSpec.RoutesInOwnTerritory and true;
  canUseRoutesByOwner[-1] = attritionSpec.RoutesInNeutralTerritory and true;

  return canUseRoutesByOwner;
end


-- Construct a DistanceCalculator for attrition distance for the given player using the settings in a UA_UnitAttritionSets.
-- Distances calculated for attrition roughly correspond to unit movement distance in the base game, but differ in some ways:
--   Embarkation is allowed everywhere only for lakes.  Entering an ocean must be done at a city or harbor.
...
--     except when capturing it).  For attrition distance the supply train in some sense can move through other player's units and 
--     cities.
--   The full cost of moving between hexes is always considered.  In game you can always move a unit that starts with full movements
...
--   Similar to the above, there is no idea of turns and having insufficient movement points to make a move this turn.
-- Arguments:
--  player: A Player object
--  spec: Specification for distance calculations in the form of an entry from GameInfo.UA_UnitAttritionSets

--TODO:
-- Embarking at harbors.
-- Can't move through enemy cities/blockades.
function MakeAttritionDistanceCalculator(player, attritionSpec) 
  local diploInfo = ExposedMembers.DiplomacyHelper.GetDiplomacyInfo(player:GetID());
  ...
  local function plotDistance(plot, nextPlot, direction)
    ...
    -- Never seen plot.  Then we can't know how to route our implicit support units through it.
    if not visibility:IsRevealed(nextPlot:GetX(), nextPlot:GetY()) then 
      return UNREACHABLE_DISTANCE; 
    end

    -- Compute distance to next hex considering terrain, feature, and improvement impediments.
    ...
    -- Embarking can only be done at a city center or a harbor for coast/ocean tiles.
    if plot:IsWater() ~= nextPlot:IsWater() then 
      ...
      elseif nextPlot:IsCity() and routeUse[nextPlot:GetOwner()] then
        return distance + CITY_EMBARK_COST;
      elseif IsHarbor(plot) then
        return distance + HARBOR_EMBARK_COST;
      elseif IsHarbor(nextPlot) then
        return distance + HARBOR_EMBARK_COST;
      else
        return UNREACHABLE_DISTANCE;
      end
  ...
  if attritionSpec.CityStateSuzerainProvidesSupport then
    for _, minor in ipairs(PlayerManager.GetAliveMinors()) do
      if diploInfo:GetDiplomaticState(minor:GetID()) == SUZERAIN_DIPLO_STATE_INDEX then
